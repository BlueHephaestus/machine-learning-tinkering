...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        if test_data: 
            ...
            test_accuracy_check_interval = 5
            a_test_accuracy = []
            a_test_x = list(range(1, test_accuracy_check_interval+1))
            eta_decrease_rate = 2#factor we decrease by
            #So if we have 3.0 initially with test_accuracy_check_interval == 10, we stop at .000003(3e-6)
            eta_stop_threshold = eta * math.pow(eta_decrease_rate, -6)
        ...
        for j in xrange(epochs):
            ...
            if test_data:
                ...
                print "Epoch {0}: {1} / {2}".format(j, test_data_accuracy, n_test)

                a_test_accuracy.append(test_data_accuracy)

                if len(a_test_accuracy) >= test_accuracy_check_interval:
                    ...
                    test_accuracy_avg_slope = (test_accuracy_check_interval*sigma(list_product(a_test_x, a_test_accuracy)) - sigma(a_test_x) * sigma(a_test_accuracy))/(test_accuracy_check_interval*sigma([x**2 for x in a_test_x])*1.0 - (sigma(a_test_x))**2)
                    if test_accuracy_avg_slope < 1:
                        eta /= eta_decrease_rate
                        print "Reducing eta by factor of {0} to {1}".format(eta_decrease_rate, eta)
