...
def load_data_shared(filename="../data/mnist.pkl.gz", normalize_x=False):
    ...
    training_data[0] = np.resize(training_data[0], (100, 83521))
    training_data[1] = np.resize(training_data[1], (100,))
    for index, val in enumerate(training_data[1]):
        if val > 5:
            training_data[1][index] = 1
        else:
            training_data[1][index] = 0
    training_data = (training_data[0], training_data[1])

    test_data = list(test_data)
    test_data[0] = np.resize(test_data[0], (100, 83521))
    test_data[1] = np.resize(test_data[1], (100,))
    for index, val in enumerate(test_data[1]):
        if val > 5:
            test_data[1][index] = 1
        else:
            test_data[1][index] = 0
    ...
    validation_data = list(validation_data)
    validation_data[0] = np.resize(validation_data[0], (100, 83521))
    validation_data[1] = np.resize(validation_data[1], (100,))
    for index, val in enumerate(validation_data[1]):
        if val > 5:
            validation_data[1][index] = 1
        else:
            validation_data[1][index] = 0
...
class Network(object):
    ...
    def __init__(self, layers, mini_batch_size):
        ...
        #self.params = [velocity for velocity in self.velocities]
        self.params = [param for layer in self.layers for param in layer.params]
        ...
        for epoch in xrange(epochs):
            for minibatch_index in xrange(num_training_batches):
                ...
                if (iteration+1) % num_training_batches == 0:
                    training_cost = np.mean([train_mb_cost(j) for j in xrange(num_training_batches)])
                    ...
                    print "Epoch %i" % (epoch)
                    print "\tTraining Cost: %f OR MAYBE %f" % (cost_ij, training_cost)
                    ...
                    if validation_accuracy > best_validation_accuracy:
                        ...
                        if test_data:
                            ...
                            test_accuracy *= 100
                            print "\tThe corresponding test accuracy is %f%%" % (test_accuracy)
        print("Finished training network.")
        print "Best training accuracy: %f%% (not necessarily same epoch as the following" % (best_training_accuracy)
        print"Best validation accuracy of %f%% obtained at epoch %i" % (best_validation_accuracy, best_epoch)
        print"Corresponding test accuracy of %f%%" % (test_accuracy)
