...
comparison_title="Cost function comparisons"
comparison_file="cost_comparison"
'''
...
if update_output:
    ...
    config_count += 1
    net = network_dev2.Network([784, 30, 10], output_filename=comparison_file, softmax=False, cost=network_dev2.log_likelihood_cost, weight_init=network_dev2.large_weight_initializer)
    net.SGD(training_data, 
            epochs, #epochs
            10,#m
            0.5,#eta
    ...
    config_count += 1
    net = network_dev2.Network([784, 30, 10], output_filename=comparison_file, softmax=False, cost=network_dev2.log_likelihood_cost2, weight_init=network_dev2.large_weight_initializer)
    net.SGD(training_data, 
            epochs, #epochs
            10,#m
            0.5,#eta
...
if graph_output:
    ...
    if subplot_seperate_configs:
        #Don't combine the configs into one, so we go to new row for each one, making the columns our output types
        ...
        #We combine into one and don't go to new rows
        fig = tools.make_subplots(rows=1, cols=output_types, shared_xaxes=False, shared_yaxes=False) 
        #row_num = 1
        col_num = 1
        for trace_index, trace in enumerate(traces):
            if np.mod(trace_index, (len(traces)/configs)) == 0 and trace_index > 0:
            ...
            for output_type in traces[trace_index]:
                for run in output_type:
                    fig.append_trace(run, 1, col_num)
                col_num+=1
